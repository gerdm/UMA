...
Suppose that at $t=0$ we have a stock $S_0$ that can go up by an up factor of $1.20$, or down by a factor of $0.80$. Assume further that the $\mathbb{P}$ measure probability for an up move is $\frac{3}{4}$, finally, suppose $r=0$. Being consistent with the notation used, $u=1.20$, $d=0.80$, $p = \frac{3}{4}$ and $ 1 - p = \frac{1}{4}$. What we want is to price is a call option on $S_0$ with strike price $k=100$.Denote this claim, at $t=4$, $f$, and $S_n$ as the value of $S$ at maturity. We can represent this claim as:
